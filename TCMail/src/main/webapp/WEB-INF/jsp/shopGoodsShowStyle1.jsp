	...
	<div class="lib_ds">
		...
		<div class="sdu_you">
			<div class="xians_fangs">
				<div class="dsfd_d">
					<span>显示：</span>
					<span><a href="toshopGoodsShowStyle1?id=${shop.id}&oreason=commnum" class="losd_14 da_15 df_15"></a></span>
					<span><a href="toshopGoodsShowStyle2?id=${shop.id}&oreason=commnum" class="losd_14 ds_15"></a></span> 
				</div>
				<div class="dsfd_d">
					<span>排序：</span>
					<span class="dgf_zy"><a href="ShowStyle1?id=${shop.id}&oreason=commnum" class="lod_ds" id="renqi" >人气</a><em class="${param.oreason eq 'commnum' ?'df':'' }"></em></span>
					<span class="dgf_zy"><a href="ShowStyle1?id=${shop.id}&oreason=ordernum" class="lod_ds" >销量</a><em class="${param.oreason eq 'ordernum' ?'df':'' }"></em></span>
					<span class="dgf_zy"><a href="ShowStyle1?id=${shop.id}&oreason=regtime" class="lod_ds">新品</a><em class="${param.oreason eq 'regtime' ?'df':'' }"></em></span>
					<span class="dgf_zy"><a href="ShowStyle1?id=${shop.id}&oreason=price" class="lod_ds" >价格</a><em class="${param.oreason eq 'price' ?'df':'' }"></em></span>
					<span class="dgf_zy"><a href="ShowStyle1?id=${shop.id}&oreason=fav" class="lod_ds">收藏</a><em class="${param.oreason eq 'fav' ?'df':'' }"></em ></span>
				</div>
			</div>
						...
						<c:forEach items="${PageBean.items }" var="g">
							<li>
								<div class="lieb_neir_kuang">
									<div class="lieb_img">
										<a href="lootbuy?gid=${g.id }"><img src="${g.image }" style="width:260px;height:200px;"></a>
										<div class="p_focus"><a class="J_focus" href="#"><i></i>关注</a></div>
									</div>
									<div class="lieb_text">
										<div class="p_price">
							              	<strong class="J_price"><em>¥</em><i>${g.price }</i><em class="shangp_yuanj zuo_ji">¥</em><i class="shangp_yuanj">${g.price }</i></strong> 
							            </div>
									</div>
									<div class="shangp_biaot_"><a href="lootbuy?gid=${g.id }">${g.name}</a></div>
